...
- **Dependency Resolution** -- When importing a skill, cross-plugin skill dependencies and MCP server requirements are detected and offered for auto-install. Bulk imports resolve dependencies across all selected skills. Meta-orchestrator skills (e.g. `using-superpowers`) from transitive dependency repos are automatically included since they must always be active.
...
- **MCP Server Support** -- Discovers MCP server configurations from `plugin.json` (inline object configs or string path references, including `mcp_servers` snake_case variant) with `.mcp.json` fallback. Writes `.vscode/mcp.json` with proper stdio/HTTP configs and secret input detection.
- **TreeView Sidebar** -- A dedicated activity bar panel groups plugins by marketplace, shows sync status with icons, and provides inline actions and right-click context menus for import/remove/embed. Dependency repos are nested under a virtual "Dependencies" folder with skill-to-skill cross-references shown as descriptions. Redirect-only marketplaces (all plugins via source URL) are displayed with their transitive sub-repos. Hover any item for a description tooltip. Click "Open Source Repository" to visit a marketplace or plugin on GitHub. Local cache plugins show only local-relevant actions; remote-only commands like "Remove Marketplace" and "Open Source Repository" are hidden for local items.
- **Bulk Import / Remove** -- Import or remove all compatible skills and MCP servers from a plugin or marketplace in one action via "Import All" and "Remove All from Project" context menus, with modal confirmation and cancellable progress notification. LM conversions run in parallel batches of 5 with a weighted progress bar showing conversion count (e.g. "Converting skills 5/20...").
...
5. **Import skills and MCP servers** -- Click the download icon on any available skill or MCP server, right-click for more options, or use "Import All" on a plugin or marketplace node to bulk import everything at once.
...
7. **Optionally sign in to GitHub** using the "Sign in to GitHub" command for access to private repos and higher rate limits.
...
| `copilotSkillBridge.importAllSkills` | Import All | Bulk import all compatible skills from a plugin |
...
| `copilotSkillBridge.importAllFromMarketplace` | Import All | Bulk import all compatible skills and MCP servers from a marketplace |
...
| `copilotSkillBridge.installInClaude` | Install in Claude Code | Install a skill's plugin into Claude Code's plugin cache |
| `copilotSkillBridge.installPluginInClaude` | Install in Claude Code | Install an entire plugin into Claude Code's plugin cache |
| `copilotSkillBridge.installAllInClaude` | Install All in Claude Code | Install all plugins from a marketplace into Claude Code's plugin cache |
